...
def binProfile(filepath):
    ...
    plt.savefig(filepath[:26]+"profVidTest3\\"+filepath[46:52]+"profile.png")
    plt.close(fig)
    
# loop through all files in given folder creating a list of file paths
...
# apply the function, binProfile, to each file in parallel
st2 = time.time()
if __name__ == '__main__':
    mp.freeze_support()
    with concurrent.futures.ThreadPoolExecutor() as executor:
        executor.map(binProfile, inputs)
print(time.time()-st2)
