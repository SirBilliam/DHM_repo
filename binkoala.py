...
def read_mat_bin(fname):
    ...
    f.close()
    shape = ((int)(kbin_header['height']), (int)(kbin_header['width']))
...
def read_mat_cplx_bin(fname):
    ...
    shape = ((int)(kcplx_header['height']), (int)(kcplx_header['width'])) 
    #print kcplx_header    
    tmp = numpy.fromfile(f, dtype='float32')
    f.close()
...
#    imag_tmp = (tmp[kcplx_header['height']*kcplx_header['width']:]).reshape(shape)
    real_tmp = (tmp[0:shape[0]*shape[1]]).reshape(shape)
    imag_tmp = (tmp[shape[0]*shape[1]:]).reshape(shape)
    #print tmp
    #print 'array = {}'.format(len(tmp)) 
